...
name = 'matching_function_3'
aspect_ratio = 'square'
filenames = [r"D:\Univerzita\Quantum scattering theory\Potential_scattering\Task 1\alpha = 4.8\Matching functions\matching_function_l=3.txt"]
x_column = 1
y_columns = [2]
show_legend = True  
legend_title = r"$l=3$"  
...
Y_axis = r'Matching function'
legends = [[''],[r"$4.7$"],[r"$4.8$"],[r"$4.9$"],[r"$10$"]]
labels = [[0],[0],[0],[0],[0]]
labels_position = [[0],[0],[0],[0],[0]]  
colors = [['blue'],['red'],['blue'],['purple'],['orange']]
markers = []
line_styles = [['-'],['-'],['-'],['-'],['-']]
line_widths = [[''],[''],[''],[''],['']]
ylog = False
...
for file_idx, filename in enumerate(filenames):
    ...
    for col_idx, y_col in enumerate(y_columns):
        ...
        discontinuities = np.abs(np.diff(y)) > threshold
        indices = np.where(discontinuities)[0]
        y[indices + 1] = np.nan
        x_filtered = x
        if label:
...
if show_legend:
    if legend_title != 0:
        ax.legend(frameon=legend_frame, title=legend_title, loc = "upper left")
    else:
